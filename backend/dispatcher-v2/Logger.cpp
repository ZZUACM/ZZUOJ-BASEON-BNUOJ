...
void Logger::log(string msg)
{
	...
	string id =
	    identifier.find(pthread_self()) ==
	    identifier.end()? "Main" : identifier[pthread_self()];
	for (vector < string >::iterator it = messages.begin();
	     it != messages.end(); ++it) {
		fprintf(fp, "%s %s[%llu]: %s\n", currentDateTime().c_str(),
			id.c_str(), (unsigned long long)pthread_self() % 10000,
			it->c_str());
	}
