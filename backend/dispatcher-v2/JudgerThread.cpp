...
void JudgerThread::updateStatistics(string runid, string result)
{
	...
	if (result.find("Accept") != string::npos) {
		...
		if (info["0"] == "1") {
			...
			db->query
			    ("UPDATE user SET total_ac = total_ac + 1 WHERE username='"
			     + db->escape(run_info["username"]) + "'");
			if (run_info["vname"] == CONFIG->Getlocal_identifier()) {
				// if it's local problem, update local_ac
				db->query
				    ("UPDATE user SET local_ac = local_ac + 1 WHERE username='"
				     + db->escape(run_info["username"]) + "'");
			}
		}
		// update problem stats
		db->query
		    ("UPDATE problem SET total_ac = total_ac + 1 WHERE pid='" +
		     db->escape(run_info["pid"]) + "'");
	} else if (result.find("Wrong Answer") != string::npos) {
		db->query
		    ("UPDATE problem SET total_wa = total_wa + 1 WHERE pid='" +
		     db->escape(run_info["pid"]) + "'");
	} else if (result.find("Runtime Error") != string::npos) {
		db->query
		    ("UPDATE problem SET total_re = total_re + 1 WHERE pid='" +
		     db->escape(run_info["pid"]) + "'");
	} else if (result.find("Presentation Error") != string::npos) {
		db->query
		    ("UPDATE problem SET total_pe = total_pe + 1 WHERE pid='" +
		     db->escape(run_info["pid"]) + "'");
	} else if (result.find("Time Limit Exceed") != string::npos) {
		db->query
		    ("UPDATE problem SET total_tle = total_tle + 1 WHERE pid='"
		     + db->escape(run_info["pid"]) + "'");
	} else if (result.find("Memory Limit Exceed") != string::npos) {
		db->query
		    ("UPDATE problem SET total_mle = total_mle + 1 WHERE pid='"
		     + db->escape(run_info["pid"]) + "'");
	} else if (result.find("Output Limit Exceed") != string::npos) {
		db->query
		    ("UPDATE problem SET total_ole = total_ole + 1 WHERE pid='"
		     + db->escape(run_info["pid"]) + "'");
	} else if (result.find("Restricted Function") != string::npos) {
		db->query
		    ("UPDATE problem SET total_rf = total_rf + 1 WHERE pid='" +
		     db->escape(run_info["pid"]) + "'");
	} else if (result.find("Compile Error") != string::npos) {
		db->query
		    ("UPDATE problem SET total_ce = total_ce + 1 WHERE pid='" +
		     db->escape(run_info["pid"]) + "'");
	}
}
	...
	while (true) {
		...
		if (current_submit) {
			...
			if (current_submit->Gettype() == NEED_JUDGE ||
				...
				// A regular run
				LOG("Load infos of Runid: " +
				    current_submit->Getid());

				// prepare file for judger
				filename =
				    Bott::RAW_FILES_DIRECTORY +
				    current_submit->Getid() + Bott::EXTENTION;
				bott = new Bott();
				bott->Setout_filename(filename);
				prepareBottForRun(bott,
						  current_submit->Getid());
				delete bott;

				// set runid result to Judging
				updateRunResult(current_submit->Getid(),
						"Judging");
				...
				try {
					...
					filename =
					    Bott::RESULTS_DIRECTORY +
					    current_submit->Getid() + "res" +
					    Bott::EXTENTION;
					socket->receiveFile(filename);
					LOG("Got result back from judger.");
				} catch(Exception & e) {
					LOG("Connection lost, requeue Runid: " +
					    current_submit->Getid());
					updateRunResult(current_submit->Getid(),
							"Judge Error & Requeued");
					return;
				}
				...
				updateStatistics(bott->Getrunid(),
						 bott->Getresult());
			} else if (current_submit->Gettype() == DO_CHALLENGE) {
				// A challenge
				LOG("Load infos of Challenge id: " +
				    current_submit->Getid());
				filename =
				    Bott::CHA_RAW_FILES_DIRECTORY +
				    current_submit->Getid() + Bott::EXTENTION;
				bott = new Bott(filename);
				prepareBottForChallenge(bott,
							current_submit->
							Getid());
				delete bott;
				...
				try {
					...
					updateChallengeResult(current_submit->
							      Getid(),
							      "Test Error & Requeued");
					return;
				}
