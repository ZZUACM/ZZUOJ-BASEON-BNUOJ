...
private:
		void dump(std::ostream & s, const Level & l,
...
inline Parser::Parser(const char *fn):f0_(fn), f_(&f0_), ln_(0) {
	 ...
	 Parser::parse(Level & l) {
		while (std::getline(*f_, line_)) {
			...
			if (line_[0] == '[') {
				...
				parent->ordered_sections.push_back(parent->
								   sections.
								   find(sname));
				parse(*lp);
			} else {
				...
				char p[] = " \t\r\n";
				while (lend > 0 && strchr(p, line_[lend]))
					--lend;
				while (rstart < line_.length()
				       && strchr(p, line_[rstart]))
					++rstart;
				if (strchr(p, line_[lend])
				    || rstart == line_.length())
					err("lvalue or rvalue is empty");
				std::pair < Level::value_map_t::const_iterator,
				    bool > res =
				    l.values.
				    insert(std::
					   make_pair(line_.substr(0, lend + 1),
						     line_.substr(rstart,
								  line_.
								  length() -
								  rstart)));
	...
	inline void
	 Parser::dump(std::ostream & s, const Level & l,
		      const std::string & sname) {
		if (!sname.empty())
			s << '\n';
		...
		if (!sname.empty())
			s << std::endl;
		for (Level::values_t::const_iterator it =
		     l.ordered_values.begin(); it != l.ordered_values.end();
		     ++it)
			s << (*it)->first << '=' << (*it)->second << std::endl;
		for (Level::sections_t::const_iterator it =
		     l.ordered_sections.begin(); it != l.ordered_sections.end();
		     ++it) {
