...
// Global config
list < Submit * > runs;				//存放runid等信息的链表
list < JudgerThread * > judgers;		//存放判题线程的链表
...
void init_network()
{
	...
	if (bind
	    (main_sockfd, (struct sockaddr *)&my_addr,
...
struct JudgerArgs {
	...
	string oj;
	 JudgerArgs(SocketHandler * _socket, string _oj):socket(_socket),
	    oj(_oj) {
}};
/**
 * Judger handler
...
void *judger_handler(void *arg)
{
	...
	JudgerThread *judger =
	    new JudgerThread(judger_info->socket, judger_info->oj);
	delete judger_info;
...
void *connection_handler(void *arg)
{
	...
	// 一有判题客户端连接进来后，根据不同的信息类型确定操作方式
	if (details[0] == CONFIG->Getjudger_string()) {	//这个是判题程序连接进来了,包括虚拟判题的程序
		...
		if (judgers.size() < MAX_JUDGER_NUMBER) {
			...
			pthread_create(&thread_id, NULL, judger_handler,
				       (void *)arg);
        ...
        ");
		delete db;
		for (vector < map < string, string > >::iterator it =
		     result.begin(); it != result.end(); ++it) {
			insert_run(new
				   Submit(NEED_JUDGE, (*it)["runid"],
					  (*it)["vname"]));
		}
...
void start_listener()
{
	...
	while (true) {
		if ((client_fd =
		     accept(main_sockfd, (struct sockaddr *)&remote_addr,
			    &sin_size)) == -1) {
...
void *dispatch(void *)
{
	...
	while (true) {
		...
		pthread_mutex_lock(&runs_mutex);
		for (list < Submit * >::iterator it = runs.begin();
		     it != runs.end(); ++it) {
			bool found = false;
			pthread_mutex_lock(&judgers_mutex);
			for (list < JudgerThread * >::iterator ij =
			     judgers.begin(); ij != judgers.end(); ++ij) {
				if ((*ij)->Getcurrent_submit() == NULL
					...
					switch ((*it)->Gettype()) {
					case NEED_JUDGE:
						LOG("Dispatched runid: " +
						    (*it)->Getid() + " for " +
						    (*it)->Getoj() +
						    " to judge");
						break;
					case DO_CHALLENGE:
						LOG("Dispatched chaid: " +
						    (*it)->Getid() + " for " +
						    (*it)->Getoj() +
						    " to judge");
						break;
...
int main()
{
	...
	// 将得到的判题任务封装成Submit对象,加入runs链表中
	for (vector < map < string, string > >::iterator it = result.begin();
	     it != result.end(); ++it) {
		insert_run(new
			   Submit(NEED_JUDGE, (*it)["runid"], (*it)["vname"]));
	}
	result.clear();
	delete db;
	...
	// 创建子线程，并设置子线程状态为detached，表示子线程
	// 结束后会自动释放所有资源
	// 这个子线程为每一个runid指定相应的判题线程
