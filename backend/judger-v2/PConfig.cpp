...
PConfig::PConfig(int pid, string prefix)
{
	...
	try {
		...
		data_checker_filename =
		    basedir + ini.top()["data_checker_filename"];
		data_checker_language =
		    atoi(ini.top()["data_checker_language"].c_str());
		solution_filename = basedir + ini.top()["solution_filename"];
		solution_language =
		    atoi(ini.top()["solution_language"].c_str());
		validator_filename = basedir + ini.top()["validator_filename"];
		validator_language =
		    atoi(ini.top()["validator_language"].c_str());
		error = false;
