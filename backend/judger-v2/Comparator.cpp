...
int Comparator::Compare()
{
	...
	if (isspj == 0) {
		...
		while (1) {
			while ((eofs =
				fscanf(standard_out, "%c", &so_char)) != EOF
			       && so_char == '\r') ;
			while ((eofp =
				fscanf(program_out, "%c", &po_char)) != EOF
			       && po_char == '\r') ;
			if (eofs == EOF || eofp == EOF)
				break;
			if (so_char != po_char) {
				detail =
				    (string) "Expected " + so_char +
				    ", Found " + po_char + " Not AC.";
				LOG((string) "Expected " + so_char +
				    ", Found " + po_char + " Not AC.");
				aced = false;
				break;
			}
		...
		while ((so_char == '\n' || so_char == '\r')
		       && ((eofs = fscanf(standard_out, "%c", &so_char)) !=
			   EOF)) ;
		while ((po_char == '\n' || po_char == '\r')
		       && ((eofp = fscanf(program_out, "%c", &po_char)) !=
			   EOF)) ;
		if (eofp != eofs)
			aced = false;
		fclose(program_out);
		...
		if (!aced) {
			...
			while (1) {
				while ((eofs =
					fscanf(standard_out, "%c",
					       &so_char)) != EOF
				       && (so_char == ' ' || so_char == '\n'
					   || so_char == '\r')) ;
				while ((eofp =
					fscanf(program_out, "%c",
					       &po_char)) != EOF
				       && (po_char == ' ' || po_char == '\n'
					   || po_char == '\r')) ;
				if (eofs == EOF || eofp == EOF)
		...
		if (spj == NULL) {
			spj = new Program;
			if (isspj == 1) {
				LOG((string) "Do SPJ, using " + "spj/" +
				    spj->Inttostring(pid) + ".cpp");
				if (!spj->Checkfile
				    ("spj/" + spj->Inttostring(pid) + ".cpp")) {
					LOG("No SPJ.");
					return JE_STATUS;
				}
				spj->Setsource(spj->Loadallfromfile("spj/" +
								    spj->
								    Inttostring
								    (pid)
								    + ".cpp"));
				...
				LOG((string) "Do SPJ, using " + "spj/" +
				    spj->Inttostring(pid) + ".java");
				if (!spj->Checkfile
				    ("spj/" + spj->Inttostring(pid) +
				     ".java")) {
					LOG("No SPJ.");
					return JE_STATUS;
				}
