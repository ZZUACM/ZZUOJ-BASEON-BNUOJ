...
{
	if (base_filename != "")
		Deletefile(base_filename + "*");
	if (Checkfile(src_filename))
		Deletefile(src_filename);
	if (Checkfile(exc_filename))
		Deletefile(exc_filename);
	if (Checkfile(out_filename))
		Deletefile(out_filename);
	if (Checkfile(err_filename))
		Deletefile(err_filename);
	if (Checkfile(res_filename))
		Deletefile(res_filename);
	if (Checkfile(in_filename))
		Deletefile(in_filename);
	if (Checkfile(cinfo_filename))
		Deletefile(cinfo_filename);
}
...
int Program::Compile(string source, int language)
{
	...
	if (language != JAVALANG)
		...
		base_filename = CONFIG->GetTmpfile_path() + "Main";
	...
	case CPPLANG:
		compile_command =
		    (string) "g++ " + src_filename + " -o " + exc_filename +
		    " -O -fno-asm -Wall -lm 2>" + cinfo_filename;
		break;
	case CLANG:
		compile_command =
		    (string) "gcc " + src_filename + " -o " + exc_filename +
		    " -O -fno-asm -Wall -lm 2>" + cinfo_filename;
		break;
	case CLANGPPLANG:
		compile_command =
		    (string) "clang++ " + src_filename + " -o " + exc_filename +
		    " -O -fno-asm -Wall -lm 2>" + cinfo_filename;
		break;
	case CLANGLANG:
		compile_command =
	...
	case FORTLANG:
		compile_command =
		    (string) "gfortran " + src_filename + " -o " +
		    exc_filename + " -Wall 2>" + cinfo_filename;
		break;
	case JAVALANG:
		compile_command =
		    (string) "javac -g:none -Xlint " + src_filename + " 2>" +
		    cinfo_filename;
		break;
	case FPASLANG:
		compile_command =
		    (string) "fpc " + src_filename + " -o" + exc_filename +
		    " -Co -Cr -Ct -Ci >" + cinfo_filename;
		break;
	case PYLANG:
		compile_command = (string)
		    "python -c \"import py_compile; py_compile.compile(\'" +
		    src_filename + "\')\" 2>" + cinfo_filename;
		break;
	case CSLANG:
		compile_command =
		    (string) "mcs " + src_filename + " -out:" + exc_filename +
		    " 2>" + cinfo_filename;
		break;
	case ADALANG:
		compile_command =
		    (string) "gnatmake " + src_filename + " 2>" +
		    cinfo_filename;
		break;
	case SMLLANG:
		compile_command =
		    (string) "mlton " + src_filename + " -output " +
		    exc_filename + " 2>" + cinfo_filename;
		break;
	case PERLLANG:
	case RUBYLANG:
		compile_command = "";
	...
	if ((cpid = fork()) == 0) {
		...
		while (1) {
			...
			if (cnt % 20 == 0)
				LOG("Compiling Used: " + Inttostring(tused));
			if (waitpid(cpid, &cstat, WNOHANG) == 0) {
				...
				LOG("kill `pstree -p " + Inttostring(cpid) +
				    " | sed 's/(/\\n(/g' | grep '(' | sed 's/(\\(.*\\)).*/\\1/' | tr \"\\n\" \" \"`");
				system(("kill `pstree -p " + Inttostring(cpid) +
					" | sed 's/(/\\n(/g' | grep '(' | sed 's/(\\(.*\\)).*/\\1/' | tr \"\\n\" \" \"`").
				       c_str());
				waitpid(cpid, &cstat, 0);
				return 2;
	...
	if (!Checkfile(exc_filename)
	    && (language == CPPLANG || language == CLANG || language == FPASLANG
		|| language == FORTLANG || language == SMLLANG
		|| language == CSLANG || language == JAVALANG
		|| language == PYLANG || language == CLANGPPLANG
		|| language == CLANGLANG)) {
		return 1;
	}
	if (!Checkfile(base_filename + ".ali") && language == ADALANG) {
		...
		compile_command =
		    (string) "gnatbind -x " + (base_filename + ".ali") + " 2>" +
		    cinfo_filename;
		system(compile_command.c_str());
		compile_command =
		    (string) "gnatlink " + (base_filename + ".ali") + " 2>" +
		    cinfo_filename;
		system(compile_command.c_str());
		if (!Checkfile(exc_filename)) {
			return 1;
...
{
	if (has_input && !Checkfile(in_filename))
		return -1;
	...
	if ((wid = fork()) == 0) {
		...
		time_limit.rlim_cur =
		    case_time_limit <
		    total_time_limit -
		    time_used ? case_time_limit : total_time_limit - time_used;
		...
		if ((pid = fork()) == 0) {
			...
			case ADALANG:
				...
				execl(exc_command.c_str(), exc_command.c_str(),
				      NULL);
				break;
			case JAVALANG:
				execl("/usr/bin/java", "java",
				      "-Djava.security.manager",
				      "-Djava.security.policy=java.policy",
				      "-client", "-cp",
				      CONFIG->GetTmpfile_path().c_str(), "Main",
				      NULL);
				break;
			case CSLANG:
				execl("/usr/bin/mono", "mono",
				      exc_filename.c_str(), NULL);
				break;
			case PERLLANG:
				execl("/usr/bin/perl", "perl",
				      src_filename.c_str(), "-W", NULL);
			case RUBYLANG:
			...
			while (1) {
				...
				time_used =
				    (rinfo.ru_utime.tv_sec +
				     rinfo.ru_stime.tv_sec) * 1000 +
				    (rinfo.ru_utime.tv_usec +
				     rinfo.ru_stime.tv_usec) / 1000;
				...
				if (memory_used <
				    getpagesize() * rinfo.ru_minflt)
					memory_used =
					    getpagesize() * rinfo.ru_minflt;
				if (WIFEXITED(runstat)) {
					LOG((string) "Used time: " +
					    Inttostring(time_used));
					LOG((string) "Used Memory: " +
					    Inttostring(memory_used));
					LOG((string) "Run status: " +
					    Inttostring(WEXITSTATUS(runstat)));
					if (check_exit_status
					    && WEXITSTATUS(runstat) != 0)
				...
				if (reg.orig_eax == SYS_execve && !excuted)
					excuted = true;
				else {
					if (language == JAVALANG) {
...
#else
				//printf("System call:%ld\n",reg.orig_rax);
				if (reg.orig_rax == SYS_execve && !excuted)
					excuted = true;
				else {
					if (language == JAVALANG) {
		...
		while (1) {
			...
			tused = case_nowv.tv_sec - case_startv.tv_sec;
			if (cnt % 20 == 0)
				LOG("Running Used: " + Inttostring(tused));
		...
		while (fin.fail())
			fin.open(res_filename.c_str(), fstream::in);
...
{
	if (vmlang[language])
		compile_time_limit = GENERAL_COMPILE_TIME * 3;
	if (source == "") {
		source = this->source;
	}
	if (language == -1) {
