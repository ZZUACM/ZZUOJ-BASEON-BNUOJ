...
Bott::Bott(string filename)
{
	...
	while (fin >> name) {
		if (name == "__SOURCE-CODE-BEGIN-LABLE__")
			...
			cha_detail =
			    parseUntil("__CHALLENGE-DETAIL-END-LABLE__");
		else {
			getline(fin, value);
			infos[name] = value;
//            LOG(name + " : " + value);
...
void Bott::toFile()
{
	...
	if (type == RESULT_REPORT) {
		fprintf(fp,
			"<type> %d\n<runid> %d\n<memory_used> %d\n<time_used> %d\n<result> %s\n",
			RESULT_REPORT, runid, memory_used, time_used,
			result.c_str());
