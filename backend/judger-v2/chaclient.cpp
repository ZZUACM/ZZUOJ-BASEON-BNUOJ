...
int check_data(int pid, string in_filename, string result_filename)
{
	...
	checker->Setsource(Loadallfromfile
			   (problem->Getdata_checker_filename()));
...
void dochallenge()
{
	...
	if (set_data(inpfile)) {
		...
		retbott.Setcha_detail("Generator Failed: " +
				      datagen->Getresult());
		if (datagen != NULL)
			delete datagen;
	} else {
		string check_result_filename =
		    CONFIG->GetTmpfile_path() + tmpnam();
		int check_stat =
		    check_data(bott->Getpid(), inpfile, check_result_filename);
		if (check_stat == 1) {
			retbott.Setcha_result("Challenge Error");
			retbott.Setcha_detail("Data Check Failed: " +
					      checker->Getresult() +
					      "\nChecker Output Detail:\n--------------------------\n"
					      +
					      Loadallfromfile
					      (check_result_filename) +
					      "\n--------------------------\nInvalid Data!!");
			...
			while (stdprogram->Getresult() == "Compile Error") {
				...
				stdprogram->Run();
				if (stdprogram->Getresult() != "Normal")
					stdres = 1;
				else
					stdres = 0;
			}
			...
			int usrres =
			    run_program(usrprogram, inpfile, bott->Getsrc(),
					bott->Getlanguage());
			...
			if (stdres != 0) {
				...
				retbott.Setcha_detail
				    ("Standard Program Failed: " +
				     stdprogram->Getresult());
			} else if (usrres != 0) {
				retbott.Setcha_result("Challenge Success");
				string newf = "cha_" + tmpnam();
				copyfile(stdprogram->Getin_filename(),
					 "testdata/" +
					 Inttostring(bott->Getpid()) + "/" +
					 newf + ".in");
				copyfile(stdprogram->Getout_filename(),
					 "testdata/" +
					 Inttostring(bott->Getpid()) + "/" +
					 newf + ".out");
				retbott.Setcha_detail("Standard: " +
						      stdprogram->Getresult() +
						      "  User: " +
						      usrprogram->Getresult());
			} else {
				cmp = new Comparator;
				cmp->Setin_filename(inpfile);
				cmp->Setout_filename(usrprogram->Getout_filename
						     ());
				cmp->Setstdout_filename
	...
	retbott.Setout_filename("cha_results/" +
				Inttostring(retbott.Getcha_id()));
	retbott.toFile();
	send_result(retbott.Getout_filename());
}

/*
 * 获取题号为pid的输入数据名字，并检测是否存在输出数据
 */
bool getalldata(int pid, vector < string > &files, int type)
{
	...
	string base;
	if (type == DO_PRETEST)
		system(("ls testdata/" + Inttostring(pid) + "/pretest_*.in >" +
			tmp_file).c_str());
	else
		system(("ls testdata/" + Inttostring(pid) + "/*.in >" +
			tmp_file).c_str());
	string tmp_string = Loadallfromfile(tmp_file);
	system(("rm " + tmp_file).c_str());
	strcpy(buffer, tmp_string.c_str());
	//从tmp_string字符串中以空白字符为分割，读出字符串
	//这里得到的是输入文件名
	char *pch;
	pch = strtok(buffer, "\t\n\r ");
	while (pch != NULL) {
...
void dojudge(int type)
{
	...
	if (!getalldata(bott->Getpid(), in_files, type) || in_files.size() == 0) {
		...
		retbott.Setout_filename("results/" +
					Inttostring(retbott.Getrunid()));
		retbott.toFile();
		// 将结果发送回获取任务程序
		send_result(retbott.Getout_filename());
		return;
	...
	if (type == NEED_JUDGE)
		...
		usrprogram->Settotal_time_limit(bott->Getcase_limit() *
						in_files.size());
	...
	if (has)
		...
		if (type == DO_PRETEST)
			retbott.Setresult("Pretest Passed");
		else
			retbott.Setresult("Accepted");
	} else if (peed)
...
void dointeractive()
{
	...
	usrprogram->SetValidator_source(Loadallfromfile
					(problem->Getvalidator_filename()));
...
int main(int argc, char *argv[])
{
	...
	while (1) {
		try {
			...
			sock->receiveFile(CONFIG->GetTmpfile_path() +
					  "temp.bott");
		}
		catch(Exception & e) {
			if (sock) {
		...
		if (bott->Gettype() == DO_CHALLENGE)
			dochallenge();
		else if (bott->Gettype() == NEED_JUDGE
			 || bott->Gettype() == DO_TESTALL
