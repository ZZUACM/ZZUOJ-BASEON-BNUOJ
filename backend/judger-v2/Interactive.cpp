...
{
	if (validator_base_filename != "")
		Deletefile(validator_base_filename + "*");
	if (Checkfile(validator_src_filename))
		Deletefile(validator_src_filename);
	if (Checkfile(validator_exc_filename))
		Deletefile(validator_exc_filename);
	if (Checkfile(validator_err_filename))
		Deletefile(validator_err_filename);
	if (Checkfile(validator_cinfo_filename))
		Deletefile(validator_cinfo_filename);
}
...
int Interactive::Excution()
{
	...
	runtime.rlim_max = runtime.rlim_cur =
	    CONFIG->GetInteractive_max_run_time() + EXTRA_RUNTIME;
	pid_t wid;
	if ((wid = fork()) == 0) {
		setpgid(0, 0);
		pid_t pgrp = getpid();
		...
		if ((vpid = fork()) == 0) {
			...
			if ((pid = fork()) == 0) {
				...
				case ADALANG:
					exc_command =
					    (string) "./" + exc_filename;
					execl(exc_command.c_str(),
					      exc_command.c_str(), NULL);
					break;
				case JAVALANG:
				...
				close(rtovPipe[0]);
				close(rtovPipe[1]);
				if (language < MIN_LANG_NUM
				    || language > MAX_LANG_NUM
				    || language == VCLANG
				    || language == VCPPLANG) {
				...
				while (1) {
					...
					if (WIFSIGNALED(runstat)
						...
						LOG((string) "Run status: " +
						    Inttostring(runstat));
						LOG((string)
						    "Run status to signal: " +
						    Inttostring(runstat >> 8));
			...
			if ((pvid = fork()) == 0) {
				...
				case CSLANG:
					execl("/usr/bin/mono", "mono",
					      validator_exc_filename.c_str(),
					      NULL);
					break;
				case PERLLANG:
					execl("/usr/bin/perl", "perl",
					      validator_src_filename.c_str(),
					      "-W", NULL);
				case RUBYLANG:
					execl("/usr/bin/ruby", "ruby",
					      validator_src_filename.c_str(),
		...
		if (result == "" || result == " ") {
			...
			LOG((string) "Validator raw exit status: " +
			    Inttostring(rstat));
			LOG((string) "Validator exit status: " +
			    Inttostring(WEXITSTATUS(rstat)));
