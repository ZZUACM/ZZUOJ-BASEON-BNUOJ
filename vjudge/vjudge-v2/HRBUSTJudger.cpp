...
void HRBUSTJudger::login()
{
	...
	string post =
	    (string) "m=User&a=login&ajax=1&user_name=" +
...
int HRBUSTJudger::submit(Bott * bott)
{
	...
	string post =
	    (string) "jumpUrl=&language=" + bott->Getlanguage() +
...
Bott *HRBUSTJudger::getStatus(Bott * bott)
{
	...
	while (true) {
		...
		curl_easy_setopt(curl, CURLOPT_URL, ((string)
						     "http://acm.hrbust.edu.cn/index.php?m=Status&a=showStatus&problem_id="
						     +
						     escapeURL(bott->Getvid()) +
						     "&user_name=" +
						     escapeURL(info->GetUsername
							       ()) +
						     "&language=" +
						     escapeURL(bott->Getlanguage
							       ())).c_str());
		performCurl();
...
string HRBUSTJudger::getCEinfo(Bott * bott)
{
	...
	prepareCurl();
	curl_easy_setopt(curl, CURLOPT_URL,
			 ("http://acm.hrbust.edu.cn/index.php?m=Status&a=showCompileError&run_id="
