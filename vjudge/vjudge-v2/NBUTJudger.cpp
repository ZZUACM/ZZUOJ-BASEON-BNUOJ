...
void NBUTJudger::login()
{
	...
	string post =
	    "username=" + escapeURL(info->GetUsername()) + "&password=" +
...
int NBUTJudger::submit(Bott * bott)
{
	...
	string post =
	    (string) "id=" + bott->Getvid() + "&language=" +
...
Bott *NBUTJudger::getStatus(Bott * bott)
{
	...
	while (true) {
		...
		prepareCurl();
		curl_easy_setopt(curl, CURLOPT_URL,
				 ("http://ac.nbutoj.com/Problem/status.xhtml?username="
		...
		if (html.find("Error Occurred") != string::npos ||
		    ...
		    string::npos
		    || !RE2::PartialMatch(html, "(?s)<tbody.*?(<tr>.*?</tr>)",
					  &status)) {
...
string NBUTJudger::getCEinfo(Bott * bott)
{
	prepareCurl();
	curl_easy_setopt(curl, CURLOPT_URL,
			 ("http://ac.nbutoj.com/Problem/viewce.xhtml?submitid="
