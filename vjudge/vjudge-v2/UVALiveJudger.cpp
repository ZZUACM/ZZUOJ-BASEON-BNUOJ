...
void UVALiveJudger::login()
{
	...
	curl_easy_setopt(curl, CURLOPT_URL,
			 "https://icpcarchive.ecs.baylor.edu/index.php?option=com_comprofiler&task=login");
	string post =
	    hiddenParams + "username=" + escapeURL(info->GetUsername()) +
	...
	if (html.find("alert(\"") != string::npos ||
	    ...
	    html.find("You are not authorized to view this page!") !=
	    string::npos) {
...
int UVALiveJudger::submit(Bott * bott)
{
	...
	prepareCurl();
	curl_easy_setopt(curl, CURLOPT_URL,
			 "https://icpcarchive.ecs.baylor.edu/index.php?option=com_onlinejudge&Itemid=25&page=save_submission");
	...
	// check submit status
	string html = loadAllFromFile(tmpfilename);
	if (html.find("You have to select a programming language.") !=
	    string::npos
	    || html.find("The selected problem ID does not exist.") !=
	    string::npos
...
Bott *UVALiveJudger::getStatus(Bott * bott)
{
	...
	while (true) {
		...
		if (html.find("<b>One or more following ERROR(s) occurred.") !=
		    string::npos
		    || html.find("The page is temporarily unavailable") !=
		    string::npos
...
string UVALiveJudger::getCEinfo(Bott * bott)
{
	prepareCurl();
	curl_easy_setopt(curl, CURLOPT_URL,
			 ("https://icpcarchive.ecs.baylor.edu/index.php?option=com_onlinejudge&Itemid=9&page=show_compilationerror&submission="
