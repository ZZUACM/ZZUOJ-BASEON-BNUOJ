...
void CFJudger::login()
{
	...
	string post = (string) "csrf_token=" + csrf +
	    ...
	    "&password=" + info->GetPassword() + "&_tta=" + getttaValue();
...
string CFJudger::getActionUrl()
{
	...
	string url;
	if (!RE2::PartialMatch
	    (html, "<form class=.*submit-form.*action=\"(.*?)\"", &url)) {
...
int CFJudger::submit(Bott * bott)
{
	...
	if (!RE2::PartialMatch
	    (bott->Getvid(), "([0-9]*)(.*)", &contest, &problem)) {
	...
	curl_formadd(&formpost, &lastptr,
		     ...
		     CURLFORM_COPYCONTENTS, contest.c_str(), CURLFORM_END);
	curl_formadd(&formpost, &lastptr,
		     CURLFORM_COPYNAME, "submittedProblemIndex",
		     CURLFORM_COPYCONTENTS, problem.c_str(), CURLFORM_END);
	curl_formadd(&formpost, &lastptr,
	...
	curl_formadd(&formpost, &lastptr,
		     ...
		     CURLFORM_COPYCONTENTS, source.c_str(), CURLFORM_END);
	curl_formadd(&formpost, &lastptr,
	...
	curl_easy_setopt(curl, CURLOPT_URL, ((string)
					     "http://codeforces.com/problemset/submit?csrf_token="
					     + csrf).c_str());
	...
	// check submit status
	string html = loadAllFromFile(tmpfilename);
	if (html.find("You have submitted exactly the same code before") !=
	    string::npos) {
	...
	} else if (html.find("<span class=\"error for__source\">") !=
		   string::npos) {
...
Bott *CFJudger::getStatus(Bott * bott)
{
	...
	while (true) {
		...
		if (!RE2::PartialMatch
		    (html, "(?s)<tr.*first-row.*?(<tr.*?</tr>)", &status)) {
			throw Exception("Failed to get status row.");
		}
		// get result
		if (!RE2::PartialMatch(status, "(?s)status-cell.*?>(.*?)</td>",
				       &result)) {
	...
	if (result_bott->Getresult() != "Accepted"
		...
		result_bott->Setce_info(getVerdict
					(contest,
					 result_bott->Getremote_runid()));
	}
	return result_bott;
}
...
string CFJudger::convertResult(string result)
{
	...
	if (result.find("Idleness") != string::npos
	    || result.find("Memory limit exceeded") != string::npos)
		return "Memory Limit Exceed";
	if (result.find("Denial of") != string::npos
	    || result.find("Judgement failed") != string::npos)
		return "Judge Error";
	return trim(result);
}
...
{k
	...
	string post =
	    (string) "submissionId=" + bott->Getremote_runid() +
	    "&csrf_token=" + csrf;
	curl_easy_setopt(curl, CURLOPT_POSTFIELDS, post.c_str());
	performCurl();
...
string CFJudger::getVerdict(string contest, string runid)
{
	prepareCurl();
	curl_easy_setopt(curl, CURLOPT_URL,
			 ((string) "http://codeforces.com/contest/" + contest +
			  "/submission/" + runid).c_str());
	performCurl();
	...
	for (hcxselect::Selector::const_iterator it = selector.begin();
	     it != selector.end(); ++it) {
		string content =
		    html.substr((*it)->data.offset(), (*it)->data.length());
		if (content.find("<div  class=\"error\">") != string::npos)
			return content;
	}
