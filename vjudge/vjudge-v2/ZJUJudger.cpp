...
void ZJUJudger::login()
{
	...
	string post =
	    "handle=" + escapeURL(info->GetUsername()) + "&password=" +
...
Bott *ZJUJudger::getStatus(Bott * bott)
{
	...
	while (true) {
		...
		prepareCurl();
		curl_easy_setopt(curl, CURLOPT_URL,
				 ("http://acm.zju.edu.cn/onlinejudge/showRuns.do?contestId=1&idStart="
				  + bott->Getremote_runid() + "&idEnd=" +
		...
		// get first row
		if (!RE2::PartialMatch
		    (html, "(?s)(<tr class=\"rowOdd\">.*?</tr>)", &status)) {
			continue;
		}
		// get result
		if (!RE2::PartialMatch(status,
				       "(?s)<td class=\"runJudgeStatus\".*?<span.*?>(.*?)</span>",
				       &result)) {
		...
		if (isFinalResult(result)) {
			...
			if (result == "Compile Error") {
				...
				if (!RE2::PartialMatch(status,
						       "(?s)showJudgeComment\\.do\\?submissionId=([0-9]*)",
...
string ZJUJudger::getCEinfo(Bott * bott)
{
	prepareCurl();
	curl_easy_setopt(curl, CURLOPT_URL,
			 ("http://acm.zju.edu.cn/onlinejudge/showJudgeComment.do?submissionId="
			  + submission_id_for_ce).c_str());
	performCurl();
