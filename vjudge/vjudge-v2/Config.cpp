...
Config::Config()
{
	try {
		...
		// Parse judger info
		judger_info.clear();
		for (map < string, INI::Level >::iterator it =
		     ini.top().sections.begin(); it != ini.top().sections.end();
		     ++it) {
			string oj = it->first;
			int max_wait_time =
			    atoi(it->second["max_wait_time"].c_str());
			for (map < string, INI::Level >::iterator ij =
			     it->second.sections.begin();
			     ij != it->second.sections.end(); ++ij) {
				judger_info.push_back(JudgerInfo
						      (oj, ij->first,
						       ij->second["username"],
						       ij->second["password"],
						       max_wait_time));
