...
void OpenJudgeJudger::login()
{
	...
	string post =
	    "email=" + escapeURL(info->GetUsername()) + "&password=" +
...
int OpenJudgeJudger::submit(Bott * bott)
{
	...
	string post =
	    "contestId=2&problemNumber=" + bott->Getvid() + "&language=" +
	...
	string runid;
	if (!RE2::PartialMatch
	    (html, "(?s)\\/practice\\\\\\/solution\\\\\\/([0-9]*)", &runid)) {
		return SUBMIT_OTHER_ERROR;
	}
...
Bott *OpenJudgeJudger::getStatus(Bott * bott)
{
	...
	while (true) {
		...
		prepareCurl();
		curl_easy_setopt(curl, CURLOPT_URL,
				 ("http://poj.openjudge.cn/practice/solution/" +
		...
		if (status.find("Error Occurred") != string::npos ||
		    status.find("The page is temporarily unavailable") !=
		    string::npos) {
			throw Exception("Failed to get status row.");
		}
		// get result
		if (!RE2::PartialMatch(status,
				       "(?s)<p class=\"compile-status\">.*?>(.*?)<",
				       &result)) {
