...
void SYSUJudger::login()
{
	...
	curl_easy_setopt(curl, CURLOPT_URL,
			 "http://soj.me/action.php?act=Login");
	string post =
...
int SYSUJudger::submit(Bott * bott)
{
	...
	prepareCurl();
	curl_easy_setopt(curl, CURLOPT_URL,
			 "http://soj.me/action.php?act=Submit");
	string post =
	    (string) "cid=0&language=" + bott->Getlanguage() + "&pid=" +
...
Bott *SYSUJudger::getStatus(Bott * bott)
{
	...
	while (true) {
		...
		// get result
		if (!RE2::PartialMatch(status, "(?s)status.*?:.*?\"(.*?)\"",
				       &result)) {
...
string SYSUJudger::getCEinfo(Bott * bott)
{
	prepareCurl();
	curl_easy_setopt(curl, CURLOPT_URL,
			 ("http://soj.me/compileresult.php?sid=" +
