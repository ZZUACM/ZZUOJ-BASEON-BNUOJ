...
void FZUJudger::login()
{
	...
	curl_easy_setopt(curl, CURLOPT_URL,
			 "http://acm.fzu.edu.cn/login.php?act=1");
	string post =
	    (string) "uname=" + info->GetUsername() + "&upassword=" +
	...
	//cout<<ts;
	if (html.find("Warning: Please Check Your UserID And Password!") !=
	    string::npos) {
...
int FZUJudger::submit(Bott * bott)
{
	prepareCurl();
	curl_easy_setopt(curl, CURLOPT_URL,
			 "http://acm.fzu.edu.cn/submit.php?act=5");
	string post =
	    (string) "usr=" + info->GetUsername() + "&lang=" +
...
Bott *FZUJudger::getStatus(Bott * bott)
{
	...
	while (true) {
		...
		if (html.find("Error Occurred") != string::npos ||
		    ...
		    string::npos || html.find(">Login</a>") != string::npos
		    || !RE2::PartialMatch(html, "(?s)(<tr onmouseover.*?</tr>)",
					  &status)) {
...
string FZUJudger::getCEinfo(Bott * bott)
{
	...
	prepareCurl();
	curl_easy_setopt(curl, CURLOPT_URL,
			 ("http://acm.fzu.edu.cn/ce.php?sid=" +
