...
void VirtualJudger::judge(Bott * bott, string filename)
{
	...
	if (submit_status == SUBMIT_NORMAL) {
		...
		case SUBMIT_SAME_CODE:
			generateSpecialResult(result,
					      "Judge Error (Same Code)");
			break;
		case SUBMIT_INVALID_LANGUAGE:
			generateSpecialResult(result,
					      "Judge Error (Invalid Language)");
			break;
	...
	result->save();
	log("Done judging. Result: " + result->Getresult() +
	    ", remote runid: " + result->Getremote_runid());
	delete result;
}
...
void VirtualJudger::run()
{
	...
	while (true) {
		...
		string result_filename =
		    Bott::RESULTS_DIRECTORY + bott->Getrunid();
		if (bott->Gettype() == NEED_JUDGE) {
			...
			try {
				if (bott->Getsrc().length() < MIN_SOURCE_LENGTH) {
					...
					generateSpecialResult(bott,
							      "Compile Error");
					bott->Setce_info
					    ("Source code too short, minimum length: "
...
void VirtualJudger::performCurl()
{
	...
	if (http_code >= 400) {
		throw Exception("Server failed, code: " +
				intToString(http_code));
	}
}
...
bool VirtualJudger::isFinalResult(string result)
{
	...
	if (result.find("waiting") != string::npos)
		return false;
	if (result.find("running") != string::npos)
		return false;
	if (result.find("judging") != string::npos)
		return false;
	if (result.find("presentation") == string::npos
	    && result.find("sent") != string::npos)
		return false;
	if (result.find("queu") != string::npos)
		return false;
	if (result.find("compiling") != string::npos)
		return false;
	if (result.find("linking") != string::npos)
		return false;
	if (result.find("received") != string::npos)
		return false;
	if (result.find("pending") != string::npos)
		return false;
	if (result.find("not judged yet") != string::npos)
		return false;
	if (result.find("being judged") != string::npos)
		return false;
