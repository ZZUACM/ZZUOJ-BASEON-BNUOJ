...
string WHUJudger::getSeed()
{
	...
	curl_easy_setopt(curl, CURLOPT_URL,
			 "http://acm.whu.edu.cn/land/ajax/vcode");
...
void WHUJudger::login()
{
	...
	string post =
	    "origURL=%2Fland&password=&passEnc=" + encryptedPassword +
	    "&username=" + info->GetUsername() + "&seed=" + seed +
	    "&remember=1";
	...
	if (html.find("<a href=\"/land/user/logout\">Logout</a>") ==
	    string::npos) {
...
int WHUJudger::submit(Bott * bott)
{
	...
	string post =
	    "lang=" + bott->Getlanguage() + "&problem_id=" + bott->Getvid() +
	    "&source=" + escapeURL(bott->Getsrc()) + "&submit=Submit";
	curl_easy_setopt(curl, CURLOPT_POSTFIELDS, post.c_str());
	performCurl();
	...
	if (html.find("This problem does not exist in Land") != string::npos ||
	    ...
	    html.find("<a href=\"/land/user/login\">Login</a>") !=
	    string::npos) {
...
Bott *WHUJudger::getStatus(Bott * bott)
{
	...
	while (true) {
		...
		curl_easy_setopt(curl, CURLOPT_URL, ((string)
						     "http://acm.whu.edu.cn/land/status?username="
						     + info->GetUsername() +
						     "&problem_id=" +
						     bott->Getvid() +
						     "&language=" +
...
string WHUJudger::getCEinfo(Bott * bott)
{
	prepareCurl();
	curl_easy_setopt(curl, CURLOPT_URL,
			 ("http://acm.whu.edu.cn/land/source/info?source_id=" +
