...
void CCJudger::login()
{
	...
	string post =
	    hiddenParams + "name=" + escapeURL(info->GetUsername()) + "&pass=" +
	    escapeURL(info->GetPassword()) + "&submit.x=0&submit.y=0";
...
string CCJudger::getCEinfo(Bott * bott)
{
	prepareCurl();
	curl_easy_setopt(curl, CURLOPT_URL,
			 ("http://www.codechef.com/view/error/" +
...
{
	if (result.find("wait") != string::npos)
		return "Running";
	if (result.find("wrong") != string::npos)
		return "Wrong Answer";
	if (result.find("time") != string::npos)
		return "Time Limit Exceed";
	if (result.find("runtime") != string::npos)
		return "Runtime Error";
	if (result.find("compile") != string::npos)
