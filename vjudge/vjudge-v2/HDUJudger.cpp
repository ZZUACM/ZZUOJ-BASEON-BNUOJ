...
void HDUJudger::login()
{
	...
	string post =
	    (string) "username=" + info->GetUsername() + "&userpass=" +
	...
	if (html.find("No such user or wrong password.") != string::npos ||
	    ...
	    string::npos
	    || html.find
	    ("<H1 style=\"COLOR: #1A5CC8\" align=center>Sign In Your Account</H1>")
	    != string::npos
...
int HDUJudger::submit(Bott * bott)
{
	...
	string post =
	    (string) "check=0&problemid=" + bott->Getvid() + "&language=" +
	    bott->Getlanguage() + "&usercode=" + escapeURL(bott->Getsrc());
	curl_easy_setopt(curl, CURLOPT_POSTFIELDS, post.c_str());
	performCurl();
...
Bott *HDUJudger::getStatus(Bott * bott)
{
	...
	while (true) {
		...
		curl_easy_setopt(curl, CURLOPT_URL, ((string)
						     "http://acm.hdu.edu.cn/status.php?first=&pid="
						     + bott->Getvid() +
						     "&user=" +
						     info->GetUsername() +
						     "&lang=&status=0").c_str
		...
		// get first row
		if (html.find("Connect(0) to MySQL Server failed.") !=
		    string::npos
		    || html.find("<b>One or more following ERROR(s) occurred.")
		    != string::npos
		    ||
		    ...
		    string::npos
		    || html.find("PHP: Maximum execution time of") !=
		    string::npos
		    ||
		    html.find
		    ("<H1 style=\"COLOR: #1A5CC8\" align=center>Sign In Your Account</H1>")
		    != string::npos
		    || html.find("<DIV>Exercise Is Closed Now!</DIV>") !=
		    string::npos
		    || !RE2::PartialMatch(html,
					  "(?s)<table.*?(<tr align=center.*?</tr>)",
					  &status)) {
		...
		if (isFinalResult(result)) {
			...
			if (!RE2::PartialMatch
			    (status, "(?s)([0-9]*)MS.*?([0-9]*)K", &time_used,
			     &memory_used)) {
				throw
				    Exception
...
string HDUJudger::getCEinfo(Bott * bott)
{
	...
	prepareCurl();
	curl_easy_setopt(curl, CURLOPT_URL,
			 ("http://acm.hdu.edu.cn/viewerror.php?rid=" +
