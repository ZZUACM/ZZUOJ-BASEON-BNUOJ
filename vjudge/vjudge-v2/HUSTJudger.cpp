...
void HUSTJudger::login()
{
	...
	string post =
	    (string) "username=" + info->GetUsername() + "&pwd=" +
...
int HUSTJudger::submit(Bott * bott)
{
	...
	curl_easy_setopt(curl, CURLOPT_URL,
			 "http://acm.hust.edu.cn/problem/submit");
	string post =
	    (string) "pid=" + bott->Getvid() + "&language=" +
...
Bott *HUSTJudger::getStatus(Bott * bott)
{
	...
	while (true) {
		...
		prepareCurl();
		curl_easy_setopt(curl, CURLOPT_URL,
				 ((string) "http://acm.hust.edu.cn/status?pid="
				  + bott->Getvid() + "&uid=" +
				  info->GetUsername() + "&language" +
...
string HUSTJudger::getCEinfo(Bott * bott)
{
	...
	prepareCurl();
	curl_easy_setopt(curl, CURLOPT_URL,
			 ("http://acm.hust.edu.cn/solution/source/" +
