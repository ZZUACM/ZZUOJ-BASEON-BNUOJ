...
void SPOJJudger::login()
{
	...
	string post =
	    "login_user=" + escapeURL(info->GetUsername()) + "&password=" +
...
Bott *SPOJJudger::getStatus(Bott * bott)
{
	...
	while (true) {
		...
		// get first row
		if (!RE2::PartialMatch(html, "(?s)(<tr class=\"kol.*?</tr>)",
				       &status)) {
			throw Exception("Failed to get status row.");
		}
		// get result
		if (!RE2::PartialMatch
		    (status, "(?s)statusres_([0-9]*).*?>(.*?)<br", &runid,
		     &result)) {
		...
		if (isFinalResult(result)) {
			...
			if (result == "Accepted" || result == "Runtime Error"
				...
				if (unit == "M") {
					int memory_mb =
					    stringToDouble(memory_used) * 1024 +
					    0.001;
