...
void LOJJudger::login()
{
	...
	curl_easy_setopt(curl, CURLOPT_URL,
			 "http://www.lightoj.com/login_check.php");
	string post =
	    "myuserid=" + escapeURL(info->GetUsername()) + "&mypassword=" +
	    escapeURL(info->GetPassword()) + "&Submit=Login";
	...
	while (true) {
		...
		if (isFinalResult(result)) {
			...
			if (result != "Compile Error") {
				...
				int time_ms =
				    stringToDouble(time_used) * 1000 + 0.001;
...
string LOJJudger::getCEinfo(Bott * bott)
{
	prepareCurl();
	curl_easy_setopt(curl, CURLOPT_URL,
			 ("http://www.lightoj.com/volume_showcode.php?sub_id=" +
	...
	string result;
	if (!RE2::PartialMatch
	    (info, "(?s)<textarea.*?>(.*?)</textarea>", &result)) {
		return "";
	}
	return result;
}
