...
void SGUJudger::login()
{
	...
	string post =
	    "try_user_id=" + escapeURL(info->GetUsername()) +
...
int SGUJudger::submit(Bott * bott)
{
	...
	string post =
	    (string) "id=" + escapeURL(info->GetUsername()) + "&pass=" +
	...
	// check submit status
	string html = loadAllFromFile(tmpfilename);
	if (html.find("Your solution was successfully submitted.") ==
	    string::npos) {
...
Bott *SGUJudger::getStatus(Bott * bott)
{
	...
	while (true) {
		...
		// get first row
		if (!RE2::PartialMatch(html, "(?s)(<TR class=st1.*?</TR>)",
				       &status)) {
