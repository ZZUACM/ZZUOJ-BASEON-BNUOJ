...
void PKUJudger::login()
{
	...
	string post =
	    (string) "user_id1=" + info->GetUsername() + "&password1=" +
...
int PKUJudger::submit(Bott * bott)
{
	...
	string post =
	    (string) "problem_id=" + bott->Getvid() + "&language=" +
...
Bott *PKUJudger::getStatus(Bott * bott)
{
	...
	while (true) {
		...
		curl_easy_setopt(curl, CURLOPT_URL,
				 ((string) "http://poj.org/status?problem_id=" +
				  bott->Getvid() + "&user_id=" +
...
string PKUJudger::getCEinfo(Bott * bott)
{
	...
	prepareCurl();
	curl_easy_setopt(curl, CURLOPT_URL,
			 ("http://poj.org/showcompileinfo?solution_id=" +
